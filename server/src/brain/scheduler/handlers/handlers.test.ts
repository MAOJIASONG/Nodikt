/**
 * 文件名称：handlers.test.ts
 * 文件作用：调度事件处理器测试模块，覆盖需求、规划、执行、审查和运维事件的核心状态流转。
 *
 * 主要职责：
 * 1. 构造内存仓储和处理器上下文。
 * 2. 测试各类事件处理器对实体状态和后续事件的影响。
 * 3. 验证暂停、恢复、取消、重试、归并和超时恢复等关键路径。
 *
 * 依赖模块：
 * - node:test 与 assert：Node 原生测试工具。
 * - domain：测试所需领域实体和事件类型。
 * - handlers：事件处理器注册入口。
 * - review/reconciliation：归并服务依赖。
 *
 * 注意事项：
 * - 测试应聚焦调度行为和状态结果，避免依赖真实文件系统或外部模型。
 * - 新增事件处理器时，应同步补充对应状态流转测试。
 */
...
function createContext(seed: ContextSeed = {}) {
  const repositories = {
    ...
    loadSettings: async () => seed.settings ?? settings()
  };
  ...
  const ctx = {
    ...
    dispatcher: {
      ...
      buildExecution: ({ demand: demandInput, subgoal: subgoalInput, worker: workerInput, attempt }: any) => execution({
...
test("createHandlers 会注册调度器事件处理器", () => {
  ...
  const handled = [
    ...
    EventType.EXECUTION_TIMEOUT_DETECTED,
    EventType.WORKER_HEALTH_CHECKED,
    EventType.OPS_RECOVERY_FAILED,
    EventType.OPS_ALERT
  ];
...
test("执行超时后 Ops 会在重试预算内自动创建下一次 execution", async () => {
  const { ctx, repositories, adapters, executionAdapters, adapter, stoppedExecutions } = createContext({
    demands: [demand({ state: DemandState.ACTIVE, current_phase: DemandPhase.EXECUTION })],
    subgoals: [subgoal({ state: SubgoalState.EXECUTING })],
    executions: [execution({
  ...
  const result = await onExecutionTimeoutDetected(event(EventType.EXECUTION_TIMEOUT_DETECTED, {
    timeout_seconds: 120,
    heartbeat_interval_seconds: 10,
    last_heartbeat_at: null,
  ...
  assert.equal((await repositories.executions.getById("exec_1"))?.state, ExecutionState.TIMEOUT);
  assert.equal((await repositories.subgoals.getById("subgoal_1"))?.state, SubgoalState.READY);
  assert.deepEqual(stoppedExecutions, ["exec_1"]);
  assert.deepEqual(result.events?.map((item) => item.event_type), [
    EventType.OPS_RECOVERY_ATTEMPTED,
    EventType.EXECUTION_CREATED
  ]);
  assert.equal(((result.events?.[1].payload as any).execution as Execution).attempt, 2);
});

test("执行超时超过重试预算后 Ops 会升级为告警", async () => {
  const { ctx, repositories, adapters, executionAdapters, adapter } = createContext({
    demands: [demand({ state: DemandState.ACTIVE, current_phase: DemandPhase.EXECUTION })],
    subgoals: [subgoal({ state: SubgoalState.EXECUTING })],
    executions: [execution({
      state: ExecutionState.RUNNING,
...
test("worker 健康检查失败会更新 worker 状态并发出 Ops 告警", async () => {
  const { ctx, repositories } = createContext({
    workers: [worker()]
  });

  const result = await onWorkerHealthChecked(event(EventType.WORKER_HEALTH_CHECKED, {
    worker_id: "worker_1",
    ok: false,
  ...
  const updatedWorker = await repositories.workers.getById("worker_1");
  assert.equal(updatedWorker?.status, WorkerRegistryStatus.ERROR);
  assert.equal(updatedWorker?.last_error, "adapter unavailable");
  assert.equal(result.events?.[0].event_type, EventType.OPS_ALERT);
});
