...
单次任务表现会随着 base model 演进持续提升，这不应成为系统层的核心护城河；
系统层真正难、也真正有价值的，是把 AI 变成一个可以长期托付的执行系统；
AI 的优势不只是“瞬间更强”，更是可以在时间维度上持续重复探索、不断积累、反复推进；
因此，系统应优化的不是 one-shot peak performance，而是 stable progress under unattended execution。
...
Scheduler 不一次性展开全部未来任务，只生成当前这几步最值得做的 frontier。
